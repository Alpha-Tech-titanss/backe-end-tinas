...
import { ApplyAccessCodeInput } from './dto/create-trip.input';
...
export class TripService {
  ...
  async addPlace(createPlaceInput) {
    ...
    const place = await (new this.PlaceModel({ ...createPlaceInput.place, bundle: bundle._id })).save()
  ...
  async applyTripAccessCode(applyAccessCodeInput: ApplyAccessCodeInput, user) {
    ...
    const found = await this.AccessCodeModel.find({ tripId: Types.ObjectId(applyAccessCodeInput.id), code: applyAccessCodeInput.code })
    if (found.length) {
      await this.userService.addSpecialTrip(user.id, Types.ObjectId(applyAccessCodeInput.id))
