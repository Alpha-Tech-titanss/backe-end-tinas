...
import { AddChapterInput, AddPlaceInput, ApplyAccessCodeInput, CreateAccessCodeInput, PlaceGalleryInput, TripInput } from './dto/create-trip.input';
...
export class TripResolver {
  ...
  @UseInterceptors(GraphqlFiles(['addPlaceInput.place.thumbnail', 'addPlaceInput.place.audio']))
  @Mutation(() => Trip)
  ...
  @Mutation(() => Trip)
  addImageToPlaceGallery(@Args('id', { type: () => String }) id: string, @Args('placeGalleryInput') placeGalleryInput: PlaceGalleryInput) {
    return this.tripService.addImageToPlace(id, placeGalleryInput);
  }

  @Mutation(() => AccessCode)
  createTripAccessCode(@Args('tripAccessCodeInput') tripAccessCodeInput: CreateAccessCodeInput) {
